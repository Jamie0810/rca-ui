...
import { Button, Table, FormFeedback, Input } from "reactstrap";
import {getComponentFilterList, uploadComponentFilterList, uploadTestStationFile} from "../../action/product-action";
...
class Product_StatisticEssentialMaterial extends React.PureComponent{
  state = {
    blackList:[],
    whiteList: [],
    attachment: null,
  ...
  componentDidMount() {
    ...
    getComponentFilterList(this.props.product.id, params).then(statisticData => {
      if (statisticData){
        this.setState({ 
  ...
  render() {
    ...
    return (
      <XwjAnalysisPoolIAO_Panel>
        <XwjAnalysisPoolIAO_PanelHeader caption={t('product.data_filter.crucial_component_caption')}>
          <Button
            color="dark"
            size="sm"
  ...
  saveTestStation = () => {
    ...
    this.setState({
      ...
      if (!this.state.attachmentInvalid){
        ...
        uploadComponentFilterList(this.props.product.id, formData)
          .then(code => {
            if (code === 422){
  ...
  fileDownload(){
    let downloadURL = API_SERVICE_BASE_URL +
      `/products/${this.props.product.id}/etl/filters/component/download?product=${this.props.product.product}`;
    window.open(downloadURL);
  }
}

export default withTranslation()(Product_StatisticEssentialMaterial);
