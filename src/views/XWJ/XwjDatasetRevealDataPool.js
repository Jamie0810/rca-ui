...
import { pageDataSetData } from '../../action/dataset-action';
import {padStart} from "lodash";
import {convertArrayToCSV} from "convert-array-to-csv";
import {withTranslation} from "react-i18next";

class XwjDatasetRevealDataPool extends React.PureComponent {
  state = {
    pool: null,
    currentPage: 0,
    recordLength: 10,
        ...
        <Table responsive striped className="nowrap">
          <thead>
            <tr>
              ...
              {pool.head_name.map(head => {
                return (<th key={head} className="border-top-0">{head}</th>);
              })}
            </tr>
          </thead>
          <tbody>
          {pool.row_data.map((row, index) => {
            return (
              <tr key={index}>
  ...
  fetchAllDataPool = () => {
    pageDataSetData(this.props.dataset.id, 1, this.state.pool.totalElements).then(poolData => {
      ...
      let filename = `${this.props.dataset.name}.csv`;
...
export default withTranslation()(XwjDatasetRevealDataPool);
